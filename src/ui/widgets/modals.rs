...
fn render_help_modal(f: &mut Frame, area: Rect, theme: &Theme) {
    ...
    let lines = vec![
        ...
        Line::from(vec![k("Enter"), d("Play / Drill into Album"), k("h / Esc"), d("Drill Up / Back to Albums")]),
        Line::from(vec![k("v"), d("Toggle View (Albums ↔ Flat List)"), k("g"), d("Cycle Radio Genre")]),
        Line::from(vec![k("a"), d("Add Track / Album to Queue"), k("x / c"), d("Remove Item / Clear Queue")]),
        Line::from(""),
        ...
        Line::from(""),
        Line::from(vec![h("VIEWS, PLAYLISTS & OVERLAYS")]),
        Line::from(vec![k("l"), d("Live Synced Karaoke Lyrics"), k("w"), d("Album Cover Artwork View")]),
        Line::from(vec![k("M"), d("Mixtape Playlist Manager"), k("m"), d("Toggle Favorite Track ★")]),
        Line::from(vec![k("H"), d("Playback History (Recent Tracks)"), k("u"), d("Universal Stream Search (YT/SC)")]),
        Line::from(vec![k("/"), d("Live Filter & Search"), k("o"), d("Settings Dashboard")]),
        Line::from(vec![k("?"), d("Toggle This Help Screen"), k("F5"), d("Hot-Reload App & Config")]),
        Line::from(vec![k("q"), d("Quit Application")]),
    ];
...
fn render_mixtape_modal(f: &mut Frame, area: Rect, state: &AppState, theme: &Theme) {
    ...
    lines.push(Line::from(vec![
        Span::styled("[↑/↓]", Style::default().fg(theme.amber).add_modifier(Modifier::BOLD)),
        Span::styled(" Select   ", Style::default().fg(theme.cream)),
        Span::styled("[Enter]", Style::default().fg(theme.green_phosphor).add_modifier(Modifier::BOLD)),
        Span::styled(" Play Mixtape   ", Style::default().fg(theme.cream)),
        ...
        Span::styled("[d]", Style::default().fg(theme.red_led).add_modifier(Modifier::BOLD)),
        Span::styled(" Remove Track   ", Style::default().fg(theme.cream)),
        ...
        Span::styled(" Del Mixtape   ", Style::default().fg(theme.cream)),
        Span::styled("[Esc/M]", Style::default().fg(theme.muted)),
        Span::styled(" Close", Style::default().fg(theme.muted)),
    ]));
...
fn render_history_modal(f: &mut Frame, area: Rect, state: &AppState, theme: &Theme) {
    ...
    lines.push(Line::from(vec![
        ...
        Span::styled("[Esc/H] ", Style::default().fg(theme.muted)),
        Span::styled("Close", Style::default().fg(theme.muted)),
    ]));
